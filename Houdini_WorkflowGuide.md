...
anoise(@P*k,重复次数，粗糙度，1);    //噪波函数；重复次数为int型，决定叠加多少层噪波，层数越多细节越丰富；粗糙度为float型，一般0.5，控制各层细节之间的振幅衰减比率（趋近0纹理越平滑粗大形状主导，0.5每层振幅是上层一半产生自然分形感，趋近1高频细节几乎不衰减纹理越粗糙毛刺感越强）；k为对纹理的缩放；返回值介于0到0.67（可以认为是0.5）；返回类型是向量或者浮点数；如果anoise优先进行乘法，自动返回浮点数,一般乘2返回黑白纹理；返回类型根据左值判断；纹理偏移为对@P进行移动,+set(x,y,z)；alligator噪波
...
onoise(@P*k,重复次数，粗糙度，1);     //噪波函数；重复次数为int型，决定叠加多少层噪波，层数越多细节越丰富；粗糙度为float型，一般0.5，控制各层细节之间的振幅衰减比率（趋近0纹理越平滑粗大形状主导，0.5每层振幅是上层一半产生自然分形感，趋近1高频细节几乎不衰减纹理越粗糙毛刺感越强）；k为对纹理的缩放；返回值介于-1到1；返回类型是向量或者浮点数；如果anoise优先进行乘法，自动返回浮点数；返回类型根据左值判断；纹理偏移为对@P进行移动,+set(x,y,z)；original perlin噪波
...
setpointgroup(几何体句柄, 组名, 点序号, 值, 模式);        //设置点的组成员关系；几何体句柄为int型，通常传0（自身）；组名为string型，不存在会自动创建；点序号为int型（@ptnum）；值为int型，1表示加入组，0表示移出组；模式为string型，可选"set"（默认，直接赋值）、"toggle"（反转）、"add"（加入）、"remove"（移除）；返回int型，0表示成功
setprimgroup(几何体句柄, 组名, 面序号, 值, 模式);         //设置面的组成员关系；几何体句柄为int型，通常传0（自身）；组名为string型，不存在会自动创建；面序号为int型（@primnum）；值为int型，1表示加入组，0表示移出组；模式为string型，同setpointgroup；返回int型，0表示成功
setvertexgroup(几何体句柄, 组名, 面序号, 顶点局部序号, 值, 模式); //设置顶点的组成员关系；几何体句柄为int型，通常传0；组名为string型；面序号为int型，该顶点所属的面的序号；顶点局部序号为int型，该顶点在所属面中的局部编号（从0开始）；值为int型，1加入0移出；模式为string型，同setpointgroup；返回int型，0表示成功
setedgegroup(几何体句柄, 组名, 点序号1, 点序号2, 值, 模式); //设置边的组成员关系；边由两个端点确定；几何体句柄为int型，通常传0；组名为string型；点序号1和点序号2为int型，构成该边的两个端点的序号；值为int型，1加入0移出；模式为string型，同setpointgroup；返回int型，0表示成功
inpointgroup(几何体句柄, 组名, 点序号);                    //查询某点是否属于指定组；几何体句柄为int型，通常传0；组名为string型；点序号为int型（@ptnum）；返回int型，1表示在组内，0表示不在组内
...
**attribtransfer节点**（即 attributetransfer）

- 将输入端1的属性按距离传递到输入端0的几何体上
  - 输入端0：目标几何体（接收属性的模型）
  - 输入端1：源几何体（提供属性的模型）
- `pointattributes` / `primitiveattributes` / `vertexattributes` / `detailattributes`：分别填写要传递的点/面/顶点/detail 层级属性名，多个属性用空格分隔，留空则不传递该层级
- `distancethreshold`（距离阈值）：目标几何体上的点/面若与源几何体的最近距离超过该值，则不接收属性（属性值保持原值或为默认值）；阈值越大，传递范围越广
- `kernel`：插值核函数，控制距离对属性权重的影响方式
  - `metaball`（默认）：基于距离平滑衰减插值，距离越近权重越大
  - `none`：直接取最近点的属性值，无插值
- 典型用途：
  - 将高精度模型的 `@Cd`（颜色）/`@N`（法线）/自定义属性烘焙到低精度模型上
  - 配合 **nearpoint** / **xyzdist** 函数的节点版替代方案，无需写 `vex`
  - 粒子发射后将静态模型的属性传递给粒子
...
- `shape`选项/`force`选项
  ...
  - `turbulence`纹理噪波
    ...
    - `turbulence`强度
      - 强度越大，效果越明显
      - 可对强度 k 帧，使湍流噪波随时间由弱变强或由强变弱，例如爆炸初期强度高、扩散后期逐渐减弱
    - `swirlsize`纹理大小/频率
      - 越大，纹理越整体
      - 越小，纹理越细碎
  ...
  - `disturbance`纹理噪波
    ...
    - `disturbance`强度
      - 强度越大，越明显
      - 以5为单位往上设置强度
      - 可对强度 k 帧，使扰动随时间变化，例如烟雾上升初期扰动弱保持形态，中后期加强扰动产生破碎感
...
- **popwind节点**
  ...
  - 支持`vex`
    ...
    - `wind`是风场
      ...
      - ```c
        wind=vex代码（返回三维向量类型）;				//只设置风场的方向
        ```
  
      - 风场的大小，噪波等通过属性窗口设置，勾选`guide`可视化力场线
...
- **staticobject节点**
  ...
  - `collisions`选项
    ...
    - `rbdsolver`的`volume`中模式设置为`volumesample`
      - 主动碰撞体是烟雾或者粒子时需要设置为`volumesample`模式并提高`uniformdivisions`精度
      - `proxyvolume`需要指定`sop`模型的`distance`体积

- **staticsolver节点**

  - 刚体解算节点
  - 和**popsolver节点**`merge`在一起后连到**output节点**，使得粒子与刚体发生碰撞
...
- **pyrosolver（sparse）节点**
  ...
  - `simulation`选项
    ...
    - timescale
      - 解算内部的时间缩放系数：值为 `1` 时正常速度；大于 `1` 解算加速（烟雾运动更快）；小于 `1` 解算减速（慢动作效果）
      - 具体机制：仅改变每帧推进的**时间步长**（timestep），所有依赖时间步长的物理量（密度/温度/速度场的平流、浮力积累、冷却衰减等）随之等比缩放，从而产生快慢效果；力本身的强度参数不变，但每帧内作用的时间区间变了
    - calculatespeedfield
      - 勾选计算并记录速度属性
  - `flames`选项
...
- 动画提取或冻结帧
- 读取前n帧的数据
- 手动k帧进行变速：在`frame`选项处手动打关键帧，将当前时间轴的帧映射到源动画的帧；例如在第1帧和第5帧分别设置`frame`值为1和10，相当于将原本10帧的动画压缩到5帧内播完，实现加速效果；反之拉长则减速
- 属性视窗中的`frame`选项处写时间函数
  - `method`为by `frame`时，写帧数（如`$F-n`代表往后推移n帧，`$F+n`代表往前推移n帧）
...
- 只能修改已存在的属性场，若不存在属性场，需要先创建属性场（通过**vdbfrompolygon**等节点先生成体积，再填充属性值）
